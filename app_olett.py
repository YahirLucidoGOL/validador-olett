...
def extraer_info_sat(texto):
    """Buscador de ultra-precisión diseñado para el formato real del SAT"""
    # Limpieza: Convertimos múltiples espacios en uno solo para facilitar la búsqueda
    ...
    # C. PERIODO (Detección por partes para evitar el N/A)
    ...
    if tiene_acuse:
        # E. EXTRACCIÓN DE MONTOS (Búsqueda por proximidad)
        ...
        # 3. TOTAL FINAL (MÉTODO BLINDADO)
        # Buscamos la línea de captura (20 caracteres) y el número que le sigue al final de la página
        # Ejemplo: 042618JA760048890425 1524994
        banco_match = re.search(r'([A-Z0-9]{20})\s+(\d{3,})', texto_limpio)
        
        if banco_match:
            # Si encontramos la cadena bancaria, tomamos ese número y le damos formato
            monto_raw = banco_match.group(2)
            datos['Total'] = f"${int(monto_raw):,}"
        else:
            # Respaldo: Buscar "TOTAL A PAGAR" y saltar hasta el siguiente número con comas
    ...
    for arc in archivos_subidos:
        ...
        for doc in docs:
            with pdfplumber.open(doc) as pdf:
                # Pegamos el texto de todas las páginas con un espacio intermedio
    ...
    for op, docs in grupos.items():
        ...
        if det and acu:
            if det['RFC'] == acu['RFC'] and det['Periodo'] == acu['Periodo']:
                ...
                st.error("❌ DISCREPANCIA: RFC o Periodo no coinciden entre documentos.")
        else:
            st.warning(f"⚠️ Operación {op} incompleta. No se detectó el par Detalle/Acuse.")
